...
// app/new-appointment.tsx
import { DateRangePickerSheet } from '@/components/DateRangePickerSheet';
import { useArtists } from '@/src/hooks/useArtists'; // DODANE: Hook do pobierania stylistek
import { useBookingData } from '@/src/hooks/useBookingData';
import { useCreateAppointment } from '@/src/hooks/useCreateAppointment';
import { DateTimeFromTo } from '@/src/types/appointment';
import { CreateAppointmentCommand } from '@/src/types/commands/CreateAppointmentCommand';
...
import { useRouter } from 'expo-router';
import React, { useMemo, useState } from 'react';
import { AnimatePresence, Avatar, Button, H4, Progress, ScrollView, Spinner, Text, TextArea, XStack, YStack } from 'tamagui';
...
export default function NewAppointmentScreen() {
  ...
  // 1. Pobieranie danych z Twojego API
  const { data: bookingData, isLoading: isLoadingBooking, error } = useBookingData();
  const { data: artists, isLoading: isLoadingArtists } = useArtists(); // Pobieramy stylistki

  // 2. Ujednolicony stan formularza
  const [currentStep, setCurrentStep] = useState(1);
  const totalSteps = 6; // Zwiększono z 5 na 6

  const [serviceId, setServiceId] = useState<number | null>(null);
  const [requestedArtistId, setRequestedArtistId] = useState<number | null>(null); // NOWY STAN
  const [nailSize, setNailSize] = useState<number | null>(null);
  const [variantId, setVariantId] = useState<number | null>(null);
  const [addonIds, setAddonIds] = useState<number[]>([]);
  const [notes, setNotes] = useState('');
  ...
  const handleSubmit = () => {
    const command: CreateAppointmentCommand = {
      requestedArtistId: requestedArtistId, // Podłączone pod wybór użytkownika
      userId: 1, // Pamiętaj o podmianie na dynamiczne w przyszłości
  ...
  // 5. Ekrany ładowania / błędów
  if (isLoadingBooking || isLoadingArtists) {
    return (
      <YStack flex={1} justifyContent="center" alignItems="center" backgroundColor="$background">
        <Spinner size="large" color="#FF2A85" />
        <Text marginTop="$2" color="$gray11">Pobieram dane salonu...</Text>
      </YStack>
    );
  }
      ...
      <ScrollView flex={1} contentContainerStyle={{ flexGrow: 1 }}>
        <AnimatePresence>
          <YStack padding="$4" gap="$5" flex={1}>
            ...
            {/* --- NOWY KROK 2: WYBÓR ARTYSTY --- */}
            {currentStep === 2 && (
              <YStack gap="$5" enterStyle={{ opacity: 0, x: 20 }}>
                <YStack>
                  <Text fontSize="$8" fontWeight="bold">Kogo wybierasz?</Text>
                  <Text color="$gray10">Masz swoją ulubioną stylistkę? Jeśli nie, zdaj się na nas!</Text>
                </YStack>

                <YStack gap="$3">
                  {/* Opcja: Ktokolwiek (Wizyta nieprzypisana) */}
                  <Button
                    size="$5"
                    justifyContent="flex-start"
                    borderWidth={requestedArtistId === null ? 2 : 1}
                    borderColor={requestedArtistId === null ? '#FF2A85' : '$borderColor'}
                    backgroundColor={requestedArtistId === null ? '#FFF0F5' : '$background'}
                    onPress={() => { setRequestedArtistId(null); nextStep(); }}
                  >
                    <Ionicons name="people-outline" size={24} color={requestedArtistId === null ? '#FF2A85' : 'gray'} />
                    <Text color={requestedArtistId === null ? '#FF2A85' : '$color'} fontWeight={requestedArtistId === null ? 'bold' : 'normal'} marginLeft="$2">
                      Dowolna Stylistka (Najszybszy termin)
                    </Text>
                  </Button>
                  ...
                  {artists?.map((artist) => (
                    <Button
                      key={artist.id}
                      size="$6" // Nieco większy przycisk, żeby pomieścić Avatar
                      justifyContent="flex-start"
                      borderWidth={requestedArtistId === artist.id ? 2 : 1}
                      borderColor={requestedArtistId === artist.id ? '#FF2A85' : '$borderColor'}
                      backgroundColor={requestedArtistId === artist.id ? '#FFF0F5' : '$background'}
                      onPress={() => { setRequestedArtistId(artist.id); nextStep(); }}
                    >
                      <Avatar circular size="$3">
                        <Avatar.Image src={artist.photoUrl || `https://i.pravatar.cc/150?u=${artist.id}`} />
                        <Avatar.Fallback backgroundColor="$gray5" />
                      </Avatar>
                      <YStack marginLeft="$3" flex={1}>
                        <Text color={requestedArtistId === artist.id ? '#FF2A85' : '$color'} fontWeight={requestedArtistId === artist.id ? 'bold' : 'normal'}>
                          {artist.name}
                        </Text>
                        {artist.instagramName && (
                          <Text color="$gray10" fontSize="$2">@{artist.instagramName}</Text>
                        )}
                      </YStack>
                    </Button>
                  ))}
      ...
      <YStack padding="$4" backgroundColor="$background" borderTopWidth={1} borderColor="$borderColor">
        ...
        <Button
          ...
          disabled={
            ...
            (currentStep === 6 && requestedDates.length === 0)
          }
          opacity={
            (currentStep === 1 && !serviceId) ||
            (currentStep === 6 && requestedDates.length === 0) ? 0.5 : 1
          }
        >
          <Button.Text color="white" fontWeight="bold">
            {currentStep === totalSteps ? 'Poproś o wizytę' : 'Kontynuuj'}
